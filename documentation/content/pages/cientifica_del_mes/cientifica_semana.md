...
.nn-scientist-hint-box {
  ...
  background: #fff8f8;
  overflow: hidden;
  box-shadow: 0 3px 12px rgba(184, 20, 36, .10);
}

.nn-scientist-hint-box[hidden] {
  display: none;
}

.nn-scientist-hint-button {
  width: 100%;
  padding: 14px 18px;
  border: 0;
...
.nn-scientist-hint-button:hover {
  background: rgba(184, 20, 36, .05);
}

.nn-scientist-hint-icon {
  margin-right: 6px;
}

.nn-scientist-hint-text {
  margin: 0;
  padding: 0 20px 16px;
  color: var(--nn-text);
  font-size: .94rem;
...
<div
  class="nn-scientist-hint-box"
  id="nnScientistHintBox"
  hidden
>
  <button
    class="nn-scientist-hint-button"
  ...
  <p
    class="nn-scientist-hint-text"
    id="nnScientistHintText"
    hidden>
  </p>
</div>
...
(function () {
  ...
  const hintBox = document.getElementById("nnScientistHintBox");
  const hintButton = document.getElementById("nnScientistHintButton");
  const hintText = document.getElementById("nnScientistHintText");
  ...
  const scientist =
    ...
    window.NN_SCIENTISTS[scientistKey];

  if (!scientist) {
    game.innerHTML =
      '<div class="nn-scientist-unavailable">' +
      "<strong>No hay una científica configurada para esta semana.</strong><br>" +
      "Lunes de la semana: " + weekStart +
      "</div>";
    return;
  }
if (
  ...
  hintText.textContent = scientist.pista.trim();
  ...
  hintButton.addEventListener("click", function () {
    ...
    hintButton.innerHTML = isHidden
      ? '<span class="nn-scientist-hint-icon" aria-hidden="true">💡</span>Pista'
      : '<span class="nn-scientist-hint-icon" aria-hidden="true">💡</span>¿Quieres una pista?';
  });
}
